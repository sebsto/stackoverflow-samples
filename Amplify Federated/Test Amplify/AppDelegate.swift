...
class AppDelegate: UIResponder, UIApplicationDelegate, GIDSignInDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        // Initialize sign-in
        GIDSignIn.sharedInstance().clientID = "778860710030-mpdbm73of72fea2noha0jt4ejum6d2s6.apps.googleusercontent.com"
...
//            let familyName = user.profile.familyName
            ...
            AWSMobileClient.sharedInstance().federatedSignIn(providerName: IdentityProvider.google.rawValue, token: idToken!) { (userState, error)  in
                if let error = error {
                    ...
                    AWSMobileClient.sharedInstance().getTokens({ (token, error) in                        print("tokens: \(token?.idToken!)")
                    })
                }
            }
        }
